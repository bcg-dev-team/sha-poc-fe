        ...
        <div className="border-b border-gray-200 px-[24px] py-[24px]">
          <div className="flex items-center justify-between">
            <input
              ...
              className="h-[46px] w-[448px] rounded-[10px] border border-[#d1d5dc] bg-gray-50 px-[16px] placeholder:text-[rgba(10,10,10,0.5)]"
            />
            <div className="flex gap-[8px]">
              <button
                onClick={() => setFilter("전체")}
                className={`h-[46px] rounded-[10px] px-[24px] shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1)] transition-colors ${
                  filter === "전체" ? "bg-[#155dfc] text-white" : "border border-[#d1d5dc] bg-gray-50 text-[#364153]"
                }`}
              >
                전체
              </button>
              <button
                ...
                className={`h-[46px] rounded-[10px] px-[24px] transition-colors ${
                  filter === "대기" ? "bg-[#155dfc] text-white shadow-[0px_4px_6px_-1px_rgba(0,0,0,0.1)]" : "border border-[#d1d5dc] bg-gray-50 text-[#364153]"
                }`}
              >
                소각 대기
              </button>
            ...
            <tbody>
              {filteredData.map((item, index) => (
                <tr key={item.id} className={index !== filteredData.length - 1 ? "border-b border-gray-200" : ""}>
                  ...
                  <td className="px-[24px] py-[20px] text-right text-[#fc8815]">{item.redemptionAmount.toLocaleString()}</td>
                  <td className="px-[24px] py-[20px] text-right text-[#0fa968]">{item.tokenAmount.toLocaleString()}</td>
                  <td className="px-[24px] py-[20px]">
                    <div className="flex justify-center">
                      {item.status === "완료" ? (
                        <span className="rounded-full border border-[#8ec5ff] bg-blue-100 px-[12px] py-[4px] text-[#1447e6]">완료</span>
                      ) : (
                        <span className="rounded-full border border-[#ffd230] bg-[#fef3c6] px-[12px] py-[4px] text-[#bb4d00]">대기</span>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        <div className="border-t border-gray-200 bg-gray-50 px-[24px] py-[17px]">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-[6px]">
              <button className="h-[34px] rounded-[10px] border border-[#d1d5dc] bg-white px-[12px] text-[#364153] opacity-50">이전</button>
              <button className="h-[32px] w-[32px] rounded-[10px] bg-[#155dfc] text-white">1</button>
              <button className="h-[34px] w-[34px] rounded-[10px] border border-[#d1d5dc] bg-white text-[#364153]">2</button>
              <button className="h-[34px] w-[34px] rounded-[10px] border border-[#d1d5dc] bg-white text-[#364153]">3</button>
              <button className="h-[34px] w-[34px] rounded-[10px] border border-[#d1d5dc] bg-white text-[#364153]">4</button>
              <button className="h-[34px] w-[34px] rounded-[10px] border border-[#d1d5dc] bg-white text-[#364153]">5</button>
              <button className="h-[34px] rounded-[10px] border border-[#d1d5dc] bg-white px-[12px] text-[#364153]">다음</button>
            </div>
          </div>
        </div>
      </div>
