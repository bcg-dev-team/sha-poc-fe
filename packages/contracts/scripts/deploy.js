...
async function main() {
  ...
  const deployParams = {
    ...
    initialNAV: parseInt(process.env.INITIAL_NAV) || 100000000, // 1.0000
  ...
  console.log("  Initial NAV:", deployParams.initialNAV / 100000000, "(내부:", deployParams.initialNAV + ")");
  ...
  const deployTx = mmfToken.deploymentTransaction();
  ...
  console.log("=".repeat(60));
  console.log("\n📝 배포 결과:");
  console.log("  컨트랙트 주소:", await mmfToken.getAddress());
  ...
  const navPrecision = await mmfToken.getNAVPrecision();
  console.log("  현재 NAV:", Number(currentNAV) / Number(navPrecision), "(내부:", currentNAV.toString() + ")");
  console.log("  NAV_PRECISION:", navPrecision.toString());

  console.log("  기본 락업 기간:", Number(await mmfToken.defaultLockupPeriod()) / (24 * 60 * 60), "일");
  console.log("  총 발행량:", ethers.formatEther(await mmfToken.totalSupply()));
  ...
  // 배포 정보 저장
  const contractAddress = await mmfToken.getAddress();
  ...
  console.log("\n  3. 테스트 매입:");
  console.log(`     mmfToken.purchaseWithDT("<USER_ADDRESS>", ethers.parseEther("1000000"))`);
  console.log("\n  4. NAV 업데이트:");
  console.log(`     mmfToken.updateNAVAndRebase(100050000) // 1.00000000 → 1.00050000 (+0.05%)`);
  console.log(`     또는: mmfToken.updateNAVWithDecimals(10002365, 7) // 1.0002365`);
